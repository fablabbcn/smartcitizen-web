  ...
  angular.module('app')
    ...
    */
    ...
    function config($stateProvider, $urlServiceProvider, $locationProvider, RestangularProvider, $logProvider, $mdAriaProvider) {
      ...
      // RestangularProvider.setBaseUrl('http://192.168.0.143:3000/v0');
...
$templateCache.put('app/components/upload/csvUpload.html','<button name="csvFiles" type="file" class="md-button md-primary md-raised" ngf-select="vm.change($files, $invalidFiles)" ngf-before-model-change="vm.onSelect()" ngf-multiple="true" ngf-accept="\'application/csv,.csv\'" ngf-max-files="10" ngf-max-size="\'10MB\'" ngf-pattern="\'.csv\'"><md-tooltip md-direction="top">Analyze the CSV before upload</md-tooltip>Load & Analyze CSV</button><md-button class="md-primary md-raised" ng-click="vm.uploadData()" ng-disabled="!vm.csvFiles || vm.csvFiles.length === 0"><md-tooltip md-direction="top">Upload the checked files</md-tooltip>Upload</md-button><md-button class="md-primary md-raised md-warn" ng-click="vm.csvFiles = []" ng-if="vm.csvFiles && vm.csvFiles.length > 0">Remove all files</md-button><md-progress-linear md-mode="{{ vm.loadingType }}" ng-value="vm.loadingProgress" ng-if="vm.loadingStatus"></md-progress-linear><md-card class="bg-yellow" ng-if="vm.invalidFiles && vm.invalidFiles.length > 0"><md-card-content><md-list><md-list-item ng-repeat="invalidFile in vm.invalidFiles"><div ng-messages="invalidFile.$errorMessages"><div ng-message="pattern">{{invalidFile.name}} is not a CSV file</div><div ng-message="maxSize">{{invalidFile.name}} is too large ({{ invalidFile.size / 1000000 | number:1 }}MB)</div><div ng-message="duplicate">{{invalidFile.name}} is already on the list</div></div></md-list-item></md-list></md-card-content><md-card-actions layout="row" layout-align="end center"><md-button ng-click="vm.invalidFiles = null" class="md-warn">close</md-button></md-card-actions></md-card><md-list ng-if="vm.csvFiles && vm.csvFiles.length > 0"><md-list-item ng-class="{\'bg-green\':csvFile.success}" ng-repeat="csvFile in vm.csvFiles" class="md-2-line"><div class="md-list-item-text" layout="column"><div class="csv_file_item"><md-checkbox ng-model="csvFile.checked" ng-disabled="csvFile.success"></md-checkbox><span>{{csvFile.name}}</span> <span ng-if="csvFile.isNew && !csvFile.success" class="label">new data</span><md-button ng-click="csvFile.detailShowed = !csvFile.detailShowed" ng-if="csvFile.parseErrors || csvFile.backEndErrors" class="md-icon-button md-warn"><md-tooltip md-direction="top">Show details</md-tooltip><md-icon md-svg-icon="./assets/images/alert_icon.svg"></md-icon></md-button><md-progress-circular ng-if="csvFile.progress" md-mode="indeterminate" md-diameter="20"></md-progress-circular><span flex=""></span><md-button ng-click="vm.removeFile($index)" class="md-icon-button md-warn md-secondary"><md-icon md-svg-icon="./assets/images/delete_icon.svg"></md-icon></md-button></div><md-card class="bg-yellow" ng-if="csvFile.detailShowed"><md-card-content><md-list><md-list-item ng-repeat="error in csvFile.parseErrors">{{error.message}} <span ng-if="error.row">(at row: {{error.row}})</span></md-list-item><md-list-item ng-if="csvFile.backEndErrors">{{csvFile.backEndErrors.statusText || csvFile.backEndErrors}} {{csvFile.backEndErrors.status}} <span ng-if="csvFile.backEndErrors.data"></span>: {{ csvFile.backEndErrors.data.message || csvFile.backEndErrors.data.errors }}</md-list-item></md-list></md-card-content></md-card></div></md-list-item></md-list><div ng-if="!vm.csvFiles || vm.csvFiles.length === 0">No files uploaded yet</div>');
$templateCache.put('app/components/upload/upload.html','<section style="margin-top: 64px"><sc-csv-upload kit="vm.kit"><sc-csv-upload></sc-csv-upload></sc-csv-upload></section>');
...
$templateCache.put('app/core/animation/backdrop/noDataBackdrop.html','<md-backdrop ng-if="vm.kitWithoutData" class="md-noDataBackdrop"><div class="block info_overlay" layout="column" layout-align="center center"><div ng-if="vm.user === \'visitor\'"><h2 class="title">This kit hasn\u2019t still said a word \uD83D\uDC76</h2><p class="message"><a analytics-on="click" analytics-event="click" analytics-category="Offline Kit Comment Link" ng-click="vm.scrollToComments()">Leave a commment</a> below to let its owner know</p></div><div ng-if="vm.user === \'owner\'" class="static_page"><h2 class="title">This kit has still not posted any data \uD83D\uDD27\uD83D\uDD29\uD83D\uDD28</h2><p class="message">Please, if your kit hasen\'t published after 5 minutes check the <a ui-sref="layout.kitEdit">kit settings</a> or reach the <a href="mailto:support@smartcitizen.me">support team.</a></p></div></div></md-backdrop>');}]);